...
Client-Abgleich: ERLEDIGT als Rueckfrage. Vor dem Wechsel wird geprueft, ob der CLIENT zum Ziel passt - passt der Loader nicht, kommt die Meldung samt Modpack-Name und Download-Link, und der naechste Klick fuehrt den Wechsel trotzdem aus (60-s-Fenster). Bewusst nie ein endgueltiges Nein: der Client-Brand ist ein freier String vom Client und faelschbar (Mods wie "Rebrand" setzen ihn auf "vanilla"). Ban/Whitelist/offline/voll bleiben dagegen hart. Die Loader-Familie kommt auf der Bukkit-Lobby aus `Player#getClientBrandName()`, im Python-Hub aus dem `minecraft:brand`-Paket, das ohnehin durch die Config-Phase laeuft. Der Hub liest dort zusaetzlich das `neoforge:register`-Manifest des Clients (im echten Capture 172 Mod-IDs) und macht daraus einen Hinweis auf fehlende Mods. Das Kompass-Menue markiert unpassende Server schon vor dem Klick grau. Regelwerk: `app/services/join_match_service.py`.

RICHTIGSTELLUNG zur alten Notiz hier: "der Ping liefert bei Forge/NeoForge eine Mod-Liste" stimmt nur zur Haelfte. Bei Forge gibt es sie noch (aber abgeschnitten, also kein Vollstaendigkeitsbeleg); ab NeoForge 1.20.4 ist sie ersatzlos gestrichen. Am eigenen ATM10-Server (NeoForge 1.21.1) nachgemessen: der Ping liefert nur `isModded: true`, kein `forgeData`. Der Ping-Weg ist fuer unsere Ziele also tot - und er beschreibt ohnehin den SERVER, den der Manager aus DB und mods/-Ordner besser kennt. Die fehlende Haelfte war immer der CLIENT.

Was sich weiterhin NICHT vorab erkennen laesst: die Modpack-VERSION (Mod-IDs tragen keine Versionsnummern - ATM10 2.0.1 gegen 2.0.2 ist auf der Leitung unsichtbar, und das ist vermutlich der haeufigste echte Disconnect); ob ein Fabric-Client die richtigen Mods hat (Fabric/Quilt kuendigen nie eine Mod-Liste an); die echte MC-Version des Clients (hinter Velocity/ViaProxy ueberschrieben - deshalb wird bewusst KEINE Versionspruefung gebaut, sie wuerde ausgerechnet die alten Clients treffen, fuer die das Universal-Netz existiert); Mod-Vollstaendigkeit bei Forge <= 1.12 (mcmod.info statt mods.toml, die Ernte liefert dort leer). Ebenso ungedeckt: der allererste Join (da gibt es noch keine Lobby zum Zurueckkehren) und eine Ablehnung, die erst NACH dem Transfer passiert (z.B. ein Plugin auf dem Ziel) - dafuer braeuchte es einen echten Proxy in der Verbindung.

Naechster sinnvoller Schritt dazu: ein kleines Velocity-Plugin am selben lobby_api-Endpoint. Velocity ist die einzige Stelle, die die ECHTE Client-Version kennt UND ablehnen kann, ohne die Verbindung zu trennen.
