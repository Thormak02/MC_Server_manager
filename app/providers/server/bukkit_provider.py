...
class BukkitProvider(ServerProviderBase):
    ...
    def provision(self, request: ProvisionServerRequest, target_dir: Path) -> ProvisionResult:
        ...
        if offline_mode_enabled():
            jar_path = target_dir / "bukkit.jar"
            write_placeholder_jar(jar_path, f"bukkit-{request.mc_version}")
        ...
        helper_script = buildtools_dir / "build_bukkit.bat"
        helper_script.write_text(
            "@echo off\n"
            f"java -jar BuildTools.jar --rev {request.mc_version} --compile craftbukkit\n",
            encoding="utf-8",
        )
        notes.append(
